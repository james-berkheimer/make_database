...
class PlexData(PlexServer):
    ...
    def __init__(self, baseurl=None, token=None, session=None, timeout=None):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize PlexData due to resource not found: {e}"
            )
            raise
        except Exception as e:
            logger.critical(
                f"Failed to initialize PlexData due to unexpected error: {e}"
            )
            raise
    ...
    def _get_sections(self, section_type):
        try:
            ...
            logger.error(
                f"Failed to get sections of type {section_type} due to bad request: {e}"
            )
            raise
        except NotFound as e:
            logger.error(
        ...
        try:
            if movies:
                if db_slice:
                    libraries_db.update(
                        {
                            k: self.get_movies_db[k]
                            for k in list(self.get_movies_db.keys())[db_slice]
                        }
                    )
            ...
            if shows:
                if db_slice:
                    libraries_db.update(
                        {
                            k: self.get_shows_db[k]
                            for k in list(self.get_shows_db.keys())[db_slice]
                        }
                    )
            ...
            if music:
                if db_slice:
                    libraries_db.update(
                        {
                            k: self.get_music_db[k]
                            for k in list(self.get_music_db.keys())[db_slice]
                        }
                    )
